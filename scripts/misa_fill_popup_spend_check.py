"""Manual, interactive check for filling the MISA Add Transaction popup:
Spend tab, amount, account, date, and category, then clicking the "Lưu"
(Save & Close) button. This DOES click Save — it will create a real
transaction in the live MISA account. Not part of the automated test
suite.

Usage:
    PYTHONPATH="$(pwd)" venv/bin/python scripts/misa_fill_popup_spend_check.py
...
from app.misa.mapper import CATEGORY
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        try:
            page.click(selectors.POPUP_SPEND_TAB)
            print("Clicked Spend tab")
        except Exception as exc:
            print(f"Clicking Spend tab FAILED: {exc}")

        try:
            page.fill(selectors.POPUP_AMOUNT_SPEND_INPUT, TEST_AMOUNT)
            print(f"Filled amount = {TEST_AMOUNT}")
        except Exception as exc:
            print(f"Filling amount FAILED: {exc}")

        if client.select_account(page, TEST_ACCOUNT):
            print(f"Selected account = {TEST_ACCOUNT!r}")
        else:
            print(f"Selecting account {TEST_ACCOUNT!r} FAILED (see log above)")

        today_str = datetime.now().strftime("%d/%m/%Y %H:%M")
        try:
            page.fill(selectors.POPUP_DATE_SPEND_INPUT, today_str)
            print(f"Filled date = {today_str}")
        except Exception as exc:
            print(f"Filling date FAILED: {exc}")

        try:
            page.fill(selectors.POPUP_CATEGORY_SPEND_INPUT, CATEGORY)
            print(f"Filled category = {CATEGORY!r}")
        except Exception as exc:
            print(f"Filling category FAILED: {exc}")

        # Typing text alone only filters the dropdown - the matching option
        # must be clicked to actually commit the selection, otherwise Save's
        # validation rejects it (red border, popup stays open).
        try:
            page.click(f'{selectors.IMPORT_BUTTON_RESULT} >> text="{CATEGORY}"')
            print(f"Selected category option = {CATEGORY!r}")
        except Exception as exc:
            print(f"Selecting category option FAILED: {exc}")

        screenshot_path = f"ai/update_misa_implementation/popup_debug_{int(datetime.now().timestamp())}.png"
        page.screenshot(path=screenshot_path)
        print(f"Saved pre-save screenshot to {screenshot_path}")

        # POPUP_SAVE_AND_CLOSE_BUTTON is an unconfirmed placeholder selector
        # (guessed via exact text match on "Lưu") - this click is the live
        # verification for it. If it's wrong, this may click nothing, click
        # "Lưu và thêm" by mistake, or fail outright.
        try:
            page.click(selectors.POPUP_SAVE_AND_CLOSE_BUTTON)
            print("Clicked Save & Close button")
        except Exception as exc:
            print(f"Clicking Save & Close button FAILED: {exc}")
            input("Press Enter to close the browser...")
            browser.close()
            return 1
        ...
        result_screenshot_path = (
            f"ai/update_misa_implementation/popup_spend_result_{int(datetime.now().timestamp())}.png"
        )
